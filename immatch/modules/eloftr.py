...
from third_party.eloftr.src.loftr import LoFTR as LoFTR_, full_default_cfg, reparameter
from .base import Matching
from immatch.utils.data_io import load_gray_scale_tensor_cv


class ELoFTR(Matching):
    def __init__(self, args):
        ...
        # Load model
        conf = dict(full_default_cfg)
        conf["match_coarse"]["thr"] = self.match_threshold
        self.model = LoFTR_(config=conf)
        ckpt_dict = torch.load(args.ckpt, weights_only=False)
        ...
        self.name = f"ELoFTR_{self.ckpt_name}"
