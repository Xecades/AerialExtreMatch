...
# method_list = ["aliked_lightglue", "disk_lightglue", "sp_lightglue", "dedode", "xfeat", "xfeat_star", "xfeat_lighterglue", "loftr", "eloftr", "aspanformer", "roma"]
# method_list = ["dust3r","mast3r"]
method_list = ["vggt"]
...
config_path = "/media/guan/data/sparse/ExtreBenchmark/localization_config/config_roma_seq5.yml"
...
matching_path = os.path.join(data_path, 'mast3r_matching')
if not os.path.exists(matching_path):
    os.mkdir(matching_path)

for method in method_list:
    result_txt = data_path + '/' + method + '.txt'
    ...
    with open(result_txt, 'w') as f_result:
        for qname in tqdm(data.keys()):
            ...
            if yaw_flag:
                ...
                # if len(mathes) > 0:
                    # plot_match(ori_path, rot_img_save_path, imgr_path, mathes, matching_path, qname, rot_R, percent=1.0)
            else:
