...
class MegadepthScene:
    ...
    def __getitem__(self, pair_idx):
        ...
        K1 = torch.tensor(
            self.intrinsics[idx1].copy(), dtype=torch.float).reshape(3, 3)
        K2 = torch.tensor(
            self.intrinsics[idx2].copy(), dtype=torch.float).reshape(3, 3)
        ...
        if romatch.DEBUG_MODE:
            tensor_to_pil(im_A[0], unnormalize=True)\
                .save(f"vis/im_A.jpg")
            tensor_to_pil(im_B[0], unnormalize=True)\
                .save(f"vis/im_B.jpg")
...
class MegadepthBuilder:
    def __init__(self, data_root="data/megadepth", loftr_ignore=True, imc21_ignore=True) -> None:
        ...
        self.loftr_ignore_scenes = set(["0121.npy", "0133.npy", "0168.npy", "0178.npy", "0229.npy",
                                       "0349.npy", "0412.npy", "0430.npy", "0443.npy", "1001.npy", "5014.npy", "5015.npy", "5016.npy"])
        self.imc21_scenes = set(["0008.npy", "0019.npy", "0021.npy",
                                "0024.npy", "0025.npy", "0032.npy", "0063.npy", "1589.npy"])
    ...
    def build_scenes(self, split="train", min_overlap=0.0, scene_names=None, **kwargs):
        ...
        for scene_name in scene_names:
            ...
            scene_info = np.load(
                os.path.join(self.scene_info_root, scene_name),
                allow_pickle=True
            ).item()
            scenes.append(
                MegadepthScene(
